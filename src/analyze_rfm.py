...
- 处理 Frequency 重复值导致的 qcut 错误
- 高价值用户退款/流失率对比
- 生成 RFM 散点图保存到 image 文件夹
- 兼容 VS Code 交互式运行
"""
...
def get_base_dir():
    ...
    parser.add_argument('--base_dir', type=str, default=default_dir,
                        help="项目根目录")
    args, unknown = parser.parse_known_args()  # 忽略 --f 等未知参数
...
def compute_rfm(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        logger.warning(f"Frequency qcut 失败（重复边界）：{e}")
        logger.info("切换到 pd.cut 自定义区间")
        # 自定义区间：根据实际分布调整
        bins_f = [0, 1, 2, 5, np.inf]  # 示例：1次、2次、3-5次、6+次
        labels_f = [1, 2, 3, 4]
...
def analyze_rfm_insights(df: pd.DataFrame, rfm: pd.DataFrame, image_dir: str):
    ...
    high_lost_rate = df[high_value_mask]['is_lost'].mean() * 100 if 'is_lost' in df.columns and high_value_mask.any() else 0
    avg_lost_rate = df['is_lost'].mean() * 100 if 'is_lost' in df.columns else 0
    ...
    # 新增：Frequency 分布柱状图
    plt.figure(figsize=(10, 6))
    sns.countplot(x='Frequency', data=rfm, palette='Blues_d', 
